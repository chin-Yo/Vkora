...
namespace vkb
{
    ...
    void screenshot(RenderContext& render_context, const std::string& filename)
    {
        ...
        bool swizzle = std::find(bgr_formats.begin(), bgr_formats.end(), src_image_view.get_format()) != bgr_formats.
            end();
        ...
        //vkb::write_image(raw_data,filename,width,height,4,width * 4);
