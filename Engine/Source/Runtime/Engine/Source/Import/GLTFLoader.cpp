...
namespace vkb
{
    namespace
    {
        ...
        inline void upload_image_to_gpu(vkb::CommandBuffer& command_buffer, vkb::Buffer& staging_buffer,
                                        sg::Image& image)
        {
        ...
        inline void prepare_meshlets(std::vector<Meshlet>& meshlets, std::unique_ptr<vkb::sg::SubMesh>& submesh,
    ...
    GLTFLoader::GLTFLoader(VulkanDevice& device) :
        device{device}
    {
    }
                                                                ...
                                                                VkBufferUsageFlags additional_buffer_usage_flags)
    {
        ...
        std::string gltf_file = Paths::GetAssetFullPath(file_name);
    ...
    sg::Scene GLTFLoader::load_scene(int scene_index, VkBufferUsageFlags additional_buffer_usage_flags)
    {
        ...
        for (auto& used_extension : model.extensionsUsed)
        {
            ...
            if (it == supported_extensions.end())
            {
                ...
                if (std::find(model.extensionsRequired.begin(), model.extensionsRequired.end(), used_extension) != model
                    .extensionsRequired.end())
                {
                    throw std::runtime_error(
                        "Cannot load glTF file. Contains a required unsupported extension: " + used_extension);
                }
                else
                {
                    // Otherwise, if extension isn't required (but is in the file) then print a warning to the user
                    LOGW("glTF file contains an unsupported extension, unexpected results may occur: {}",
                         used_extension);
                }
            }
            else
        ...
        {
            std::vector<vkb::Buffer> transient_buffers;
            ...
            while (image_index < image_count && batch_size < 64 * 1024 * 1024)
            {
                ...
                Buffer stage_buffer = vkb::Buffer::create_staging_buffer(device, image->get_data());
        ...
        for (auto& gltf_texture : model.textures)
        {
            ...
            else
            {
                ...
                const VkFormatProperties fmtProps = device.get_gpu().get_format_properties(
                    images[gltf_texture.source]->get_format());
        ...
        for (auto& gltf_mesh : model.meshes)
        {
            ...
            for (size_t i_primitive = 0; i_primitive < gltf_mesh.primitives.size(); i_primitive++)
            {
                ...
                if (gltf_primitive.indices >= 0)
                {
                    ...
                    submesh->index_buffer = std::make_unique<vkb::Buffer>(device,
                                                                          index_data.size(),
                                                                          VK_BUFFER_USAGE_INDEX_BUFFER_BIT |
                                                                          additional_buffer_usage_flags,
                                                                          VMA_MEMORY_USAGE_GPU_TO_CPU);
                    submesh->index_buffer->SetDebugName(fmt::format("'{}' mesh, primitive #{}: index buffer",
                ...
                {
                    submesh->vertices_count = to_u32(
                        get_attribute_size(&model, gltf_primitive.attributes.at("POSITION")));
                }
        ...
        for (size_t animation_index = 0; animation_index < model.animations.size(); ++animation_index)
        {
            ...
            for (size_t channel_index = 0; channel_index < gltf_animation.channels.size(); ++channel_index)
            {
                ...
                {
                    LOGW("Gltf animation channel #{} has unsupported target path: {}", channel_index,
                                                        ...
                                                        VkBufferUsageFlags additional_buffer_usage_flags)
    {
        ...
        pos = reinterpret_cast<const float*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset + buffer_view.
            byteOffset]));
        ...
        if (gltf_primitive.attributes.find("NORMAL") != gltf_primitive.attributes.end())
        {
            ...
            normals = reinterpret_cast<const float*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
        ...
        if (gltf_primitive.attributes.find("TEXCOORD_0") != gltf_primitive.attributes.end())
        {
            ...
            buffer_view = model.bufferViews[accessor.bufferView];
            uvs = reinterpret_cast<const float*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
                buffer_view.byteOffset]));
        }
        ...
        if (gltf_primitive.attributes.find("COLOR_0") != gltf_primitive.attributes.end())
        {
            ...
            buffer_view = model.bufferViews[accessor.bufferView];
            colors = reinterpret_cast<const float*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
                buffer_view.byteOffset]));
        ...
        if (gltf_primitive.attributes.find("JOINTS_0") != gltf_primitive.attributes.end())
        {
            ...
            buffer_view = model.bufferViews[accessor.bufferView];
            joints = reinterpret_cast<const uint16_t*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
                buffer_view.byteOffset]));
        }
        ...
        if (gltf_primitive.attributes.find("WEIGHTS_0") != gltf_primitive.attributes.end())
        {
            ...
            buffer_view = model.bufferViews[accessor.bufferView];
            weights = reinterpret_cast<const float*>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
                buffer_view.byteOffset]));
        }
        ...
        if (storage_buffer)
        {
            ...
            vkb::Buffer stage_buffer = vkb::Buffer::create_staging_buffer(device, aligned_vertex_data);
        ...
        else
        {
            ...
            vkb::Buffer stage_buffer = vkb::Buffer::create_staging_buffer(device, vertex_data);
        ...
        if (gltf_primitive.indices >= 0)
        {
            ...
            if (storage_buffer)
            {
                ...
                vkb::Buffer stage_buffer = vkb::Buffer::create_staging_buffer(device, meshlets);
            ...
            {
                vkb::Buffer stage_buffer = vkb::Buffer::create_staging_buffer(device, index_data);
    ...
    std::unique_ptr<sg::Node> GLTFLoader::parse_node(const tinygltf::Node& gltf_node, size_t index) const
    {
        ...
        if (!gltf_node.translation.empty())
        {
            ...
            std::transform(gltf_node.translation.begin(), gltf_node.translation.end(), glm::value_ptr(translation),
        ...
        if (!gltf_node.rotation.empty())
        {
            ...
            std::transform(gltf_node.rotation.begin(), gltf_node.rotation.end(), glm::value_ptr(rotation),
                           TypeCast<double, float>{});
    ...
    std::unique_ptr<sg::PBRMaterial> GLTFLoader::parse_material(const tinygltf::Material& gltf_material) const
    {
        ...
        for (auto& gltf_value : gltf_material.values)
        {
            if (gltf_value.first == "baseColorFactor")
            {
                ...
                material->base_color_factor = glm::vec4(color_factor[0], color_factor[1], color_factor[2],
                                                        color_factor[3]);
            }
    ...
    std::unique_ptr<sg::Sampler> GLTFLoader::parse_sampler(const tinygltf::Sampler& gltf_sampler) const
    {
        ...
        Sampler vk_sampler{device, sampler_info};
    ...
    std::vector<std::unique_ptr<sg::Light>> GLTFLoader::parse_khr_lights_punctual()
    {
        ...
        {
            if (model.extensions.find(KHR_LIGHTS_PUNCTUAL_EXTENSION) == model.extensions.end() || !model.extensions.at(
                KHR_LIGHTS_PUNCTUAL_EXTENSION).Has("lights"))
            {
                return {};
            }
            ...
            for (size_t light_index = 0; light_index < khr_lights.ArrayLen(); ++light_index)
            {
                ...
                if (type != sg::LightType::Directional)
                {
                    ...
                    if (type != sg::LightType::Point)
                    {
                        ...
                        {
                            LOGE("KHR_lights_punctual extension: spot light doesn't have a 'spot' property set",
                                 gltf_light_type);
                            throw std::runtime_error(
                                "Couldn't load glTF file, KHR_lights_punctual extension is invalid");
                        }

                        properties.inner_cone_angle = static_cast<float>(khr_light.Get("spot").Get("innerConeAngle").Get
                            <double>());

                        if (khr_light.Get("spot").Has("outerConeAngle"))
                        {
                            properties.outer_cone_angle = static_cast<float>(khr_light.Get("spot").Get("outerConeAngle")
                                .Get<double>());
                        }
                        else
                        {
    ...
    tinygltf::Value* GLTFLoader::get_extension(tinygltf::ExtensionMap& tinygltf_extensions,
