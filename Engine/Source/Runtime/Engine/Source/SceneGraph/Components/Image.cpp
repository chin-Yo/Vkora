...
#include "SceneGraph/Components/Image/Ktx.h"
#include "SceneGraph/Components/Image/Stb.h"
...
namespace vkb
{
    namespace sg
    {
        ...
        Image::Image(const std::string &name, std::vector<uint8_t> &&d, std::vector<Mipmap> &&m) : Component{name},
                                                                                                   data{std::move(d)},
        ...
        void Image::create_vk_image(VulkanDevice &device, VkImageViewType image_view_type, VkImageCreateFlags flags)
        {
            ...
            vk_image_view = std::make_unique<ImageView>(*vk_image, image_view_type);
            vk_image_view->SetDebugName("View on " + get_name());
        }

        const vkb::Image &Image::get_vk_image() const
        {
            assert(vk_image && "Vulkan image was not created");
            return *vk_image;
        }

        const vkb::ImageView &Image::get_vk_image_view() const
        {
