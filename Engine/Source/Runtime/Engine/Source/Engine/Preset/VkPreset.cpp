...
ps::Texture ps::load_texture(vkb::VulkanDevice &device, const std::string& file, vkb::sg::Image::ContentType content_type)
{
    Texture texture{};
	...
	sampler_create_info.maxAnisotropy    = device.get_gpu().get_requested_features().samplerAnisotropy ? (device.get_gpu().get_properties().limits.maxSamplerAnisotropy) : 1.0f;
