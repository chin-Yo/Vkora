...
#include "Engine/SceneGraph/Components/Image/Ktx.hpp"
...
namespace scene
{
    ...
    void Image::create_vk_image(vkb::VulkanDevice& device, VkImageViewType image_view_type, VkImageCreateFlags flags)
    {
        ...
        vk_image = std::make_unique<vkb::Image>(device,
                                                ...
                                                flags);
        vk_image->SetDebugName(GetName());

        vk_image_view = std::make_unique<vkb::ImageView>(*vk_image, image_view_type);
        vk_image_view->SetDebugName("View on " + GetName());
    }
