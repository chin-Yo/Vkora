...
namespace vkb
{
          ...
          surface_extent{window.GetExtent().width, window.GetExtent().height}
    {
        if (surface != VK_NULL_HANDLE)
        {
            ...
            {
                swapchain = std::make_unique<Swapchain>(device, surface, present_mode, present_mode_priority_list,
    ...
    void RenderContext::update_swapchain(const VkExtent2D& extent, const VkSurfaceTransformFlagBitsKHR transform)
    {
        ...
        {
            LOGW(
                "Can't update the swapchains extent and surface transform. No swapchain, offscreen rendering detected, skipping.");
            return;
        }
    ...
    void RenderContext::update_swapchain(const VkImageCompressionFlagsEXT compression,
    ...
    std::shared_ptr<vkb::CommandBuffer> RenderContext::begin(vkb::CommandBufferResetMode reset_mode)
    {
    ...
    void RenderContext::submit(std::shared_ptr<vkb::CommandBuffer> command_buffer)
    {
        std::vector<std::shared_ptr<vkb::CommandBuffer>> command_buffers(1, command_buffer);
        submit(command_buffers);
    }

    void RenderContext::submit(const std::vector<std::shared_ptr<vkb::CommandBuffer>>& command_buffers)
    {
        ...
        if (swapchain)
        {
            ...
            if (result == VK_SUBOPTIMAL_KHR || result == VK_ERROR_OUT_OF_DATE_KHR)
            {
                ...
                if (swapchain_updated)
                {
                    ...
                    vkDestroySemaphore(device.GetHandle(), acquired_semaphore, nullptr);
                    acquired_semaphore = prev_frame.get_semaphore_pool().request_semaphore_with_ownership();
                                      ...
                                      VkPipelineStageFlags wait_pipeline_stage)
    {
        ...
        VK_CHECK_RESULT(queue.submit({submit_info}, fence));

        return signal_semaphore;
    }

    void RenderContext::submit(const Queue& queue,
                               const std::vector<std::shared_ptr<vkb::CommandBuffer>>& command_buffers)
    {
        std::vector<VkCommandBuffer> cmd_buf_handles(command_buffers.size(), VK_NULL_HANDLE);
        std::transform(command_buffers.begin(), command_buffers.end(), cmd_buf_handles.begin(),
    ...
    vkb::RenderFrame& RenderContext::get_active_frame()
    {
        assert(frame_active && "Frame is not active, please call begin_frame");
        assert(active_frame_index < frames.size());
    ...
    vkb::RenderFrame& RenderContext::get_last_rendered_frame()
    {
        assert(!frame_active && "Frame is still active, please call end_frame");
        assert(active_frame_index < frames.size());
        return *frames[active_frame_index];
    ...
    VulkanDevice& RenderContext::get_device()
    {
        return device;
    }
    ...
    std::vector<std::unique_ptr<vkb::RenderFrame>>& RenderContext::get_render_frames()
    {
        return frames;
    }
} // namespace vkb
