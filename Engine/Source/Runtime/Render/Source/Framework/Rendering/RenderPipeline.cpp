...
namespace vkb
{
    RenderPipeline::RenderPipeline(std::vector<std::unique_ptr<vkb::Subpass>>&& subpasses_) : subpasses{
        std::move(subpasses_)
    }
    {
        prepare();
    ...
    void RenderPipeline::add_subpass(std::unique_ptr<vkb::Subpass>&& subpass)
    {
        subpass->prepare();
        subpasses.emplace_back(std::move(subpass));
    }

    std::vector<std::unique_ptr<vkb::Subpass>>& RenderPipeline::get_subpasses()
    {
        return subpasses;
    }
    ...
    void RenderPipeline::draw(vkb::CommandBuffer& command_buffer, RenderTarget& render_target,
    ...
    std::unique_ptr<vkb::Subpass>& RenderPipeline::get_active_subpass()
    {
