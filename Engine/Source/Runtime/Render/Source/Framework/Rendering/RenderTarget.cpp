...
namespace vkb
{
    namespace
    {
        struct CompareExtent2D
        {
            ...
            {
                return !(lhs.width == rhs.width && lhs.height == rhs.height) && (lhs.width < rhs.width && lhs.height <
                    rhs.height);
            }
        };
    } // namespace
        ...
        images{std::move(images)}
    {
        ...
        auto get_image_extent = [](const Image& image)
        {
    ...
    RenderTarget::RenderTarget(std::vector<ImageView>&& image_views) :
        device{const_cast<Image&>(image_views.back().get_image()).GetDevice()},
        images{},
