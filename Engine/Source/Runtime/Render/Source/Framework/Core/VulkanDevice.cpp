...
namespace vkb
{
    VulkanDevice::VulkanDevice(PhysicalDevice& gpu,
        ...
        vkb::VulkanResource<VkDevice>{VK_NULL_HANDLE, this}, // Recursive, but valid
        ...
        resource_cache{*this}
    {
        ...
        {
            const VkQueueFamilyProperties& queue_family_property = gpu.get_queue_family_properties()[
                queue_family_index];
            ...
            is_extension_supported("VK_EXT_host_query_reset"))
        {
            auto perf_counter_features =
                gpu.get_extension_features<VkPhysicalDevicePerformanceQueryFeaturesKHR>(
                    VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PERFORMANCE_QUERY_FEATURES_KHR);
            auto host_query_reset_features =
                gpu.get_extension_features<VkPhysicalDeviceHostQueryResetFeatures>(
                    VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_HOST_QUERY_RESET_FEATURES);

            if (perf_counter_features.performanceCounterQueryPools && host_query_reset_features.hostQueryReset)
            {
                gpu.add_extension_features<VkPhysicalDevicePerformanceQueryFeaturesKHR>(
                       VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PERFORMANCE_QUERY_FEATURES_KHR)
                   .performanceCounterQueryPools = VK_TRUE;
                gpu.add_extension_features<VkPhysicalDeviceHostQueryResetFeatures>(
                        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_HOST_QUERY_RESET_FEATURES).hostQueryReset =
                    VK_TRUE;
        ...
        command_pool = std::make_unique<vkb::CommandPool>(
            *this, get_queue_by_flags(VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT, 0).get_family_index());
        fence_pool = std::make_unique<FencePool>(*this);
    }

    VulkanDevice::VulkanDevice(PhysicalDevice& gpu, VkDevice& vulkan_device, VkSurfaceKHR surface) :
    ...
    uint32_t VulkanDevice::get_memory_type(uint32_t bits, VkMemoryPropertyFlags properties,
    ...
    void VulkanDevice::add_queue(size_t global_index, uint32_t family_index, VkQueueFamilyProperties properties,
    ...
    uint32_t VulkanDevice::get_queue_family_index(VkQueueFlagBits queue_flag)
    {
        ...
        if (queue_flag & VK_QUEUE_COMPUTE_BIT)
        {
            ...
            {
                if ((queue_family_properties[i].queueFlags & queue_flag) && !(queue_family_properties[i].queueFlags &
        ...
        if (queue_flag & VK_QUEUE_TRANSFER_BIT)
        {
            ...
            {
                if ((queue_family_properties[i].queueFlags & queue_flag) && !(queue_family_properties[i].queueFlags &
                    VK_QUEUE_GRAPHICS_BIT) && !(queue_family_properties[i].queueFlags & VK_QUEUE_COMPUTE_BIT))
                {
                    return i;
                    break;
                }
    ...
    void VulkanDevice::flush_command_buffer(VkCommandBuffer command_buffer, VkQueue queue, bool free,
