...
namespace vkb
{
																						   ...
																						   handle{physical_device}
	{
		...
		VK_CHECK_RESULT(vkEnumerateDeviceExtensionProperties(get_handle(), nullptr, &device_extension_count, nullptr));
		device_extensions = std::vector<VkExtensionProperties>(device_extension_count);
		VK_CHECK_RESULT(vkEnumerateDeviceExtensionProperties(get_handle(), nullptr, &device_extension_count, device_extensions.data()));
	...
	VkBool32 PhysicalDevice::is_present_supported(VkSurfaceKHR surface, uint32_t queue_family_index) const
	{
		...
		{
			VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceSupportKHR(handle, queue_family_index, surface, &present_supported));
		}
