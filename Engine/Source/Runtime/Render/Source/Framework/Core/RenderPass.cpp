...
namespace vkb
{
                                       ...
                                       const std::vector<SubpassInfo>& subpasses)
    {
        ...
        {
            LOG_WARN("Render Pass creation: size of attachment list and load/store info list does not match: {} vs {}",
                     attachments.size(), load_store_infos.size());
        }

        auto attachment_descriptions = get_attachment_descriptions<T_AttachmentDescription>(
            attachments, load_store_infos);
        ...
        for (size_t i = 0; i < subpasses.size(); ++i)
        {
            ...
            {
                auto initial_layout = is_depth_format(attachments[i_attachment].format)
        ...
        for (size_t i = 0; i < subpasses.size(); ++i)
        {
            ...
            subpass_description.pResolveAttachments = color_resolve_attachments[i].empty()
                                                          ? nullptr
        ...
        if (subpasses.empty())
        {
            ...
            for (uint32_t k = 0U; k < to_u32(attachment_descriptions.size()); ++k)
            {
                ...
                color_attachments[0].push_back(
                    get_attachment_reference<T_AttachmentReference>(k, VK_IMAGE_LAYOUT_GENERAL));
            }
...
namespace vks
{
    ...
    VkRenderPass RenderPassBuilder::buildRaw()
    {
        VkRenderPassCreateInfo renderPassInfo{};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
        renderPassInfo.attachmentCount = static_cast<uint32_t>(m_attachments.size());
