...
namespace vkb
{
    Queue::Queue(VulkanDevice& device, uint32_t family_index, VkQueueFamilyProperties properties, VkBool32 can_present,
                 uint32_t index) : device{device},
    ...
    const VulkanDevice& Queue::get_device() const
    {
        return device;
    }
    ...
    VkResult Queue::submit(const vkb::CommandBuffer& command_buffer, VkFence fence) const
    {
