...
namespace vkb
{
		...
		const VkExtent2D &current_extent)
	{
		...
		{
			LOG_WARN("(Swapchain) Image extent ({}, {}) not supported. Selecting ({}, {}).", request_extent.width, request_extent.height, current_extent.width, current_extent.height);
		...
		const std::vector<VkPresentModeKHR> &present_mode_priority_list)
	{
		...
		auto present_mode_it = std::find(available_present_modes.begin(),
		...
		if (present_mode_it == available_present_modes.end())
		{
			...
			LOG_WARN("(Swapchain) Present mode '{}' not supported. Selecting '{}'.", to_string(request_present_mode), to_string(chosen_present_mode));
			return chosen_present_mode;
		}
		else
		{
			LOG_INFO("(Swapchain) Present mode selected: {}", to_string(request_present_mode));
		...
		const std::vector<VkSurfaceFormatKHR> &surface_format_priority_list)
	{
		...
		auto surface_format_it = std::find_if(
			...
			{
				return (surface.format == requested_surface_format.format &&
		...
		if (surface_format_it == available_surface_formats.end())
		{
			for (auto &surface_format : surface_format_priority_list)
			{
				auto surface_format_it = std::find_if(
					...
					{
						return (surface.format == surface_format.format &&
								surface.colorSpace == surface_format.colorSpace);
					});
				if (surface_format_it != available_surface_formats.end())
				{
					LOG_WARN("(Swapchain) Surface format ({}) not supported. Selecting ({}).", to_string(requested_surface_format), to_string(*surface_format_it));
		...
		{
			LOG_INFO("(Swapchain) Surface format selected: {}", to_string(requested_surface_format));
		}
		...
		VkSurfaceTransformFlagBitsKHR current_transform)
	{
		...
		LOG_WARN("(Swapchain) Surface transform '{}' not supported. Selecting '{}'.", to_string(request_transform), to_string(current_transform));
	...
	inline VkCompositeAlphaFlagBitsKHR choose_composite_alpha(VkCompositeAlphaFlagBitsKHR request_composite_alpha, VkCompositeAlphaFlagsKHR supported_composite_alpha)
	{
		...
		for (VkCompositeAlphaFlagBitsKHR composite_alpha : composite_alpha_flags)
		{
			...
			{
				LOG_WARN("(Swapchain) Composite alpha '{}' not supported. Selecting '{}.", to_string(request_composite_alpha), to_string(composite_alpha));
	...
	inline std::set<VkImageUsageFlagBits> choose_image_usage(const std::set<VkImageUsageFlagBits> &requested_image_usage_flags, VkImageUsageFlags supported_image_usage, VkFormatFeatureFlags supported_features)
	{
		...
		for (auto flag : requested_image_usage_flags)
		{
			...
			{
				LOG_WARN("(Swapchain) Image usage ({}) requested but not supported.", to_string(flag));
			}
		}
		...
		if (!validated_image_usage_flags.empty())
		{
			...
			LOG_INFO("(Swapchain) Image usage flags: {}", usage_list);
		}
		else
		{
	...
	Swapchain::Swapchain(Swapchain &old_swapchain, const VkImageCompressionFlagsEXT requested_compression, const VkImageCompressionFixedRateFlagsEXT requested_compression_fixed_rate) : Swapchain{old_swapchain,
																																																   old_swapchain.device,
																																																   old_swapchain.surface,
																																																   old_swapchain.properties.present_mode,
																																																   old_swapchain.present_mode_priority_list,
	...
	Swapchain::Swapchain(VulkanDevice &device,
						 ...
						 const VkImageCompressionFixedRateFlagsEXT requested_compression_fixed_rate) : Swapchain{*this, device, surface, present_mode, present_mode_priority_list, surface_format_priority_list, extent, image_count, transform, image_usage_flags}
	{
	}
	...
	Swapchain::Swapchain(Swapchain &old_swapchain,
						 ...
						 const std::set<VkImageUsageFlagBits> &image_usage_flags,
						 const VkImageCompressionFlagsEXT requested_compression,
						 const VkImageCompressionFixedRateFlagsEXT requested_compression_fixed_rate) : device{device},
																									   surface{surface},
																									   ...
																									   requested_compression_fixed_rate{requested_compression_fixed_rate}
	{
		...
		VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(this->device.get_gpu().get_handle(), surface, &surface_format_count, nullptr));

		std::vector<VkSurfaceFormatKHR> surface_formats(surface_format_count);
		VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(this->device.get_gpu().get_handle(), surface, &surface_format_count, surface_formats.data()));

		LOG_INFO("Surface supports the following surface formats:");
		for (auto &surface_format : surface_formats)
		{
			LOG_INFO("  \t{}", to_string(surface_format));
		}

		uint32_t present_mode_count{0U};
		VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(this->device.get_gpu().get_handle(), surface, &present_mode_count, nullptr));

		std::vector<VkPresentModeKHR> present_modes(present_mode_count);
		VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(this->device.get_gpu().get_handle(), surface, &present_mode_count, present_modes.data()));

		LOG_INFO("Surface supports the following present modes:");
		for (auto &pm : present_modes)
		{
			LOG_INFO("  \t{}", to_string(pm));
		}
		...
		if (device.is_enabled(VK_EXT_IMAGE_COMPRESSION_CONTROL_SWAPCHAIN_EXTENSION_NAME))
		{
			...
			{
				LOG_WARN("(Swapchain) Disabling default (lossless) compression, which can negatively impact performance")
			}
		}
		else
		{
			...
			{
				LOG_WARN("(Swapchain) Compression cannot be controlled because VK_EXT_image_compression_control_swapchain is not enabled")
		...
		VK_CHECK_RESULT(vkGetSwapchainImagesKHR(device.GetHandle(), handle, &image_available, nullptr));

		images.resize(image_available);

		VK_CHECK_RESULT(vkGetSwapchainImagesKHR(device.GetHandle(), handle, &image_available, images.data()));

		if (device.is_enabled(VK_EXT_IMAGE_COMPRESSION_CONTROL_SWAPCHAIN_EXTENSION_NAME) &&
			VK_IMAGE_COMPRESSION_FIXED_RATE_EXPLICIT_EXT == requested_compression)
		{
			// Check if fixed-rate compression was applied
			const auto applied_compression_fixed_rate = vkb::query_applied_compression(device.GetHandle(), images[0]).imageCompressionFixedRateFlags;

			if (applied_compression_fixed_rate != requested_compression_fixed_rate)
			{
				LOG_WARN("(Swapchain) Requested fixed-rate compression ({}) was not applied, instead images use {}",
			...
			{
				LOG_INFO("(Swapchain) Applied fixed-rate compression: {}",
						 image_compression_fixed_rate_flags_to_string(applied_compression_fixed_rate));
			}
		}
	}
	...
	VulkanDevice &Swapchain::get_device()
	{
		return device;
	}
	...
	{
		return vkAcquireNextImageKHR(device.GetHandle(), handle, std::numeric_limits<uint64_t>::max(), image_acquired_semaphore, fence, &image_index);
	}
	...
	{
		return vkb::query_applied_compression(device.GetHandle(), get_images()[0]).imageCompressionFlags;
	}

	std::vector<Swapchain::SurfaceFormatCompression> Swapchain::query_supported_fixed_rate_compression(VulkanDevice &device, const VkSurfaceKHR &surface)
	{
		...
		if (device.is_enabled(VK_EXT_IMAGE_COMPRESSION_CONTROL_SWAPCHAIN_EXTENSION_NAME))
		{
			if (device.get_gpu().get_instance().is_enabled(VK_KHR_GET_SURFACE_CAPABILITIES_2_EXTENSION_NAME))
			{
				...
				VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormats2KHR(device.get_gpu().get_handle(), &surface_info, &surface_format_count, nullptr));
				...
				VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormats2KHR(device.get_gpu().get_handle(), &surface_info, &surface_format_count, surface_formats.data()));
			...
			{
				LOG_WARN("(Swapchain) To query fixed-rate compression support, instance extension VK_KHR_get_surface_capabilities2 must be enabled")
			}
		}
		else
		{
			LOG_WARN("(Swapchain) To query fixed-rate compression support, device extension VK_EXT_image_compression_control_swapchain must be enabled")
		}
