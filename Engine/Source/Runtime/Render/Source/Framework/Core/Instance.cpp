...
#if defined(DEBUG) || defined(VK_VALIDATION_LAYERS)
...
namespace vkb
{
    namespace
    {
            ...
            void* user_data)
        {
            ...
            {
                LOG_WARN("{} - {}: {}", callback_data->messageIdNumber, callback_data->pMessageIdName,
...
#endif
        /**
         * @brief Verifies that the required Vulkan validation layers exist in the available list
         *
         * @param required List of required validation layer names
         * @param available List of available validation layer properties in the system
         * @return bool Returns true if all required validation layers are found, false otherwise
         */
        bool validate_layers(const std::vector<const char*>& required,
                             const std::vector<VkLayerProperties>& available)
        {
            // Iterate through each required layer
            for (auto layer : required)
            {
                bool found = false;
                // Check if the required layer exists in the available layers
                for (auto& available_layer : available)
                {
                    // Compare the layer names
                ...
                // If the required layer is not found, log an error and return false
                if (!found)
                {
    ...
    namespace
    {
                              ...
                              std::vector<const char*>& enabled_extensions)
        {
            bool is_available =
                std::any_of(available_extensions.begin(), available_extensions.end(),
                            [&requested_extension](auto const& available_extension)
                            {
            ...
            if (is_available)
            {
                bool is_already_enabled =
                    std::any_of(enabled_extensions.begin(), enabled_extensions.end(),
                                [&requested_extension](auto const& enabled_extension)
                                {
                                    return strcmp(requested_extension, enabled_extension) == 0;
                                });
                                  ...
                                  std::vector<VkLayerSettingEXT>& enabled_layer_settings)
        {
            ...
            bool is_available =
                std::any_of(enabled_layers.begin(), enabled_layers.end(),
                            [&requested_layer_setting](auto const& available_layer)
                            {
                                return strcmp(available_layer, requested_layer_setting.pLayerName) == 0;
                            });
            if (!is_available)
            {
                LOGW("Layer: {} not found. Disabling layer setting: {}", requested_layer_setting.pLayerName,
                     requested_layer_setting.pSettingName);
                return false;
            }

            bool is_already_enabled =
                std::any_of(enabled_layer_settings.begin(), enabled_layer_settings.end(),
                            [&requested_layer_setting](VkLayerSettingEXT const& enabled_layer_setting)
                            {
                                return (strcmp(requested_layer_setting.pLayerName,
                                               enabled_layer_setting.pLayerName) == 0) &&
                                (strcmp(requested_layer_setting.pSettingName,
                                        enabled_layer_setting.pSettingName) == 0);
                            });

            if (is_already_enabled)
            {
                LOGW("Ignoring duplicated layer setting {} in layer {}.", requested_layer_setting.pSettingName,
                     requested_layer_setting.pLayerName);
                return false;
            }

            LOGI("Enabling layer setting {} in layer {}.", requested_layer_setting.pSettingName,
                 requested_layer_setting.pLayerName);
                          ...
                          std::vector<const char*>& enabled_layers)
        {
            bool is_available =
                std::any_of(available_layers.begin(), available_layers.end(),
                            [&requested_layer](auto const& available_layer)
                            {
            ...
            if (is_available)
            {
                bool is_already_enabled =
                    std::any_of(enabled_layers.begin(), enabled_layers.end(),
                                [&requested_layer](auto const& enabled_layer)
                                {
                                    return strcmp(requested_layer, enabled_layer) == 0;
                                });
                if (!is_already_enabled)
                {
                       ...
                       uint32_t api_version)
    {
        ...
        VK_CHECK_RESULT(
            vkEnumerateInstanceExtensionProperties(nullptr, &instance_extension_count, available_instance_extensions.
                data()));

#ifdef USE_VALIDATION_LAYERS
        // Check if VK_EXT_debug_utils is supported, which supersedes VK_EXT_Debug_Report
        const bool has_debug_utils = enable_extension(
            VK_EXT_DEBUG_UTILS_EXTENSION_NAME, available_instance_extensions, enabled_extensions);
        bool has_debug_report = false;

        if (!has_debug_utils)
        {
            has_debug_report = enable_extension(
                VK_EXT_DEBUG_REPORT_EXTENSION_NAME, available_instance_extensions, enabled_extensions);
            if (!has_debug_report)
            {
		...
		bool validation_features = false;
		{
			...
			VK_CHECK_RESULT(vkEnumerateInstanceExtensionProperties(validation_layer_name, &layer_instance_extension_count, nullptr));
			std::vector<VkExtensionProperties> available_layer_instance_extensions(layer_instance_extension_count);
			VK_CHECK_RESULT(vkEnumerateInstanceExtensionProperties(validation_layer_name, &layer_instance_extension_count, available_layer_instance_extensions.data()));
        ...
        enable_extension(
            VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, available_instance_extensions, enabled_extensions);
        ...
        VK_CHECK_RESULT(vkEnumerateInstanceLayerProperties(&instance_layer_count, supported_layers.data()));
    ...
    Instance::Instance(VkInstance instance,
                       const std::vector<const char*>& externally_enabled_extensions) : handle{instance}
    {
    ...
    void Instance::query_gpus()
    {
        ...
        VK_CHECK_RESULT(vkEnumeratePhysicalDevices(handle, &physical_device_count, nullptr));
        ...
        VK_CHECK_RESULT(vkEnumeratePhysicalDevices(handle, &physical_device_count, physical_devices.data()));
