...
namespace vkb
{
    namespace
    {
        void read_subpass_info(std::istringstream& is, std::vector<SubpassInfo>& value)
        {
            std::size_t size;
            read(is, size);
            value.resize(size);
        ...
        void read_processes(std::istringstream& is, std::vector<std::string>& value)
        {
            std::size_t size;
            read(is, size);
            value.resize(size);
    ...
    void ResourceReplay::play(ResourceCache& resource_cache, ResourceRecord& recorder)
    {
        ...
        while (true)
        {
            ...
            {
                LOG_ERROR("Replay command not supported.");
            }
        }
    }
