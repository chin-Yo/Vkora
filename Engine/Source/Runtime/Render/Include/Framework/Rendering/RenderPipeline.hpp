...
namespace vkb
{
    ...
    {
    public:
        RenderPipeline(std::vector<std::unique_ptr<vkb::Subpass>>&& subpasses = {});
         ...
         */
        void add_subpass(std::unique_ptr<vkb::Subpass>&& subpass);

        std::vector<std::unique_ptr<vkb::Subpass>>& get_subpasses();

        /**
         * @brief Record draw commands for each Subpass
         */
        void draw(vkb::CommandBuffer& command_buffer, RenderTarget& render_target,
         ...
         */
        std::unique_ptr<vkb::Subpass>& get_active_subpass();
