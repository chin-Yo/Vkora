...
namespace vkb
{
	...
	{
	public:
		Queue(VulkanDevice &device, uint32_t family_index, VkQueueFamilyProperties properties, VkBool32 can_present, uint32_t index);
		...
		VkResult submit(const vkb::CommandBuffer &command_buffer, VkFence fence) const;
